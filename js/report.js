...
window.SiemensReport = (function() {
    ...
    function render(results) {
        ...
        state.tab1.data = sortByRefDes(compChanges);
        
        state.tab2.data = sortByRefDes(results.connectivityChanges);
        
        // For Tab 3 (Drill Down), we group connectivity changes by RefDes
        const groupedConn = {};
        state.tab2.data.forEach(c => {
        ...
        state.tab3.data = Object.keys(groupedConn).map(refdes => ({
            refdes: refdes,
            changes: sortByRefDes(groupedConn[refdes])
        })).sort((a,b) => compareRefDes(a.refdes, b.refdes));
        
        state.tab4.data = sortByNetName(results.netRenames);
    ...
    function sortByRefDes(rows) {
        return [...rows].sort((a, b) => {
            const refCmp = compareRefDes(a.refdes, b.refdes);
            if (refCmp !== 0) return refCmp;

            const pinCmp = comparePin(a.pin, b.pin);
            if (pinCmp !== 0) return pinCmp;

            return compareNatural(a.status, b.status);
        });
    }

    function sortByNetName(rows) {
        return [...rows].sort((a, b) => {
            const oldCmp = compareNatural(a.oldNet, b.oldNet);
            if (oldCmp !== 0) return oldCmp;
            return compareNatural(a.newNet, b.newNet);
        });
    }
    ...
    return {
        ...
        getTab1Data: () => sortByRefDes(state.tab1.filtered),
        getTab2Data: () => sortByRefDes(state.tab2.filtered),
        getTab4Data: () => sortByNetName(state.tab4.filtered),
